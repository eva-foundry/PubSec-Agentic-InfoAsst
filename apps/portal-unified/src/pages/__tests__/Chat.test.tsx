...
describe("Chat: NDJSON streaming", () => {
  it("streams agent_step / content / citations / provenance_complete and renders a grounded answer", async () => {
    ...
    await waitFor(() => expect(screen.queryAllByText(/the social benefits Act/i).length).toBeGreaterThan(0));
    ...
    await waitFor(
      () => expect(screen.getByText(/social benefits benefits are paid monthly/i)).toBeInTheDocument(),
      { timeout: 5000 },
    );
