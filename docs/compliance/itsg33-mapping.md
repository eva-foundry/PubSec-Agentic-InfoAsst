# ITSG-33 Security Control Mapping -- EVA Agentic

> Classification: Protected B | Last updated: 2026-04-15
>
> This document maps ITSG-33 security controls to EVA Agentic implementation artefacts.
...
| Control ID | Control Name | EVA Implementation | Evidence Location |
|-----------|-------------|-------------------|------------------|
...
| IA-8 | Identification and Authentication (Non-Organizational Users) | Not applicable -- EVA Agentic is internal to ESDC. All users authenticate via Entra ID organizational accounts. External access explicitly denied at SCED boundary. | `CLAUDE.md` (Hard constraints -- SCED/VNet) |
...
| SC-13 | Cryptographic Protection | Encryption at rest via Azure platform-managed keys for Cosmos DB, Blob Storage, and AI Search indexes. All data classified Protected B or lower. | `infra/modules/cosmos/`, `infra/modules/storage/` |
| SC-28 | Protection of Information at Rest | All Azure resources provisioned in Canada Central / Canada East only (data residency). Cosmos DB, Blob Storage, and AI Search enforce server-side encryption. Daily incremental backups to paired Canadian region. | `infra/environments/`, `scripts/backup/`, `services/api-gateway/app/stores/workspace_store.py` (infrastructure config) |
...
| Continuous authorization | ITSG-33 control evidence auto-collected. Certification report regenerated on each deployment. Red-team suite runs in CI. | `scripts/certify/`, `scripts/red-team/`, `evidence/` |
