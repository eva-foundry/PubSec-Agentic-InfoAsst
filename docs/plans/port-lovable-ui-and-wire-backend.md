...
- [apps/portal-unified/package.json](apps/portal-unified/package.json) — keep `@eva/portal-unified` name + `typecheck` script, take deps/version from mind-arch
...
3. **Override package.json**: restore `"name": "@eva/portal-unified"`, restore `"typecheck": "tsc --noEmit"` script, keep everything else from mind-arch.
...
**Verification after Phase A**: `npm run dev --workspace @eva/portal-unified` serves the app on `http://localhost:5173` and every route renders against mock-data exactly as `mind-arch.lovable.app` does.
...
- `stream.ts` — `streamNdjson<T>(url, init)` — async iterator over the `POST /v1/eva/chat` NDJSON body using the Fetch + `ReadableStream` + `TextDecoder` pattern, yielding parsed `ChatEvent` discriminated union (`provenance | degradation | content | agent_step | provenance_complete`).
...
2. New `src/pages/Login.tsx` — minimal form that calls `POST /v1/eva/auth/demo/login`, surfaces demo personas via `GET /v1/eva/auth/demo/users`, persists token + user to localStorage.
...
**Workspace portal**
- `pages/Catalog.tsx` — replace `WORKSPACES`/`ARCHETYPES` with `useWorkspaces()` + a new `useArchetypes()` hook. Archetypes are not yet a dedicated endpoint — start by deriving from `GET /v1/eva/workspaces` group-by `archetype` (backend enhancement tracked as follow-up).
...
**Admin portal**
- `pages/Onboarding.tsx` — 6-step wizard. Steps 1–2 (org profile, data classification) → `POST /v1/eva/admin/clients`. Step 3 (assurance level) → local state (default applied to future workspace provisioning). Step 4 (workspace templates) → `POST /v1/eva/admin/workspaces/provision` with `dry_run: true` first, confirm, then execute. Step 5 (role mapping) → patch client. Progress persisted via existing localStorage key (`aia-onboarding-progress`) now keyed off real client_id.
...
- `pages/Compliance.tsx` — audit log from `useOps().feedbackAnalytics` + a new `useAuditLog()` hook hitting `/v1/eva/ops/audit` (stub to add — see Phase F), framework chips static (compliance frameworks are configuration, not runtime data).
- `pages/RedTeam.tsx` — `useEvalArena()` for baseline table. Adversarial test runner is the one real gap: UI action calls a new `POST /v1/eva/ops/eval/challenges` endpoint that doesn't exist yet → keep current mock for now, flag as Phase F.
...
- `pages/DevOps.tsx` — `useDeployments()` for history; rollback button wired to a new `POST /v1/eva/admin/deployments/{version}/rollback` (Phase F follow-up, keep disabled with tooltip until endpoint exists).
...
- `GET /v1/eva/ops/audit` — typed audit log for Compliance page (today audit events are embedded elsewhere).
- `POST /v1/eva/ops/eval/challenges`, `GET /v1/eva/ops/eval/results` — Red Team adversarial test runner.
- `GET /v1/eva/ops/metrics/drift` — dedicated drift signals (model, prompt, corpus) for Drift page.
- `POST /v1/eva/admin/deployments/{version}/rollback` — DevOps rollback action.
...
End-to-end, run in three terminals from `/Users/marco/Documents/eva-foundry/53-EVA-Refactor`:
...
# Terminal 2 — frontend
npm run dev --workspace @eva/portal-unified   # serves on :5173

# Terminal 3 — verification
npm run typecheck --workspaces
npm run lint --workspaces
npm run test --workspace @eva/portal-unified   # vitest incl. jest-axe a11y
...
2. `/chat` — select "Internal HR Handbook", ask the prefilled question, confirm:
   ...
   - Sources panel populates with real citation metadata (`GET /v1/eva/citations/{id}` in the waterfall).
   - Confidence badge, behavioural fingerprint line, explainability text present in the final message.
3. `/models` — toggle a model off; confirm `PATCH /v1/eva/admin/models/{id}/toggle` fires; reload and persistence holds.
4. `/cost` — KPIs and chart render from `GET /v1/eva/ops/metrics/finops`.
...
- Porting to GCDS components (Government of Canada design system) — tracked as follow-up per CLAUDE.md UI stack decision.
