...
class TestAIOpsTimeseries:
    def test_default_returns_14_days(self, client: TestClient):
        body = client.get("/v1/eva/ops/metrics/aiops", headers=DAVE).json()
        assert body["days"] == 14
        assert len(body["timeseries"]) == 14

    def test_days_query_param(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/aiops?days=30", headers=DAVE
        ).json()
        assert body["days"] == 30
    ...
    def test_days_clamped_to_90(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/aiops?days=500", headers=DAVE
        ).json()
        assert body["days"] == 90
    ...
    def test_timeseries_last_day_is_today(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/aiops?days=7", headers=DAVE
        ).json()
...
class TestCalibrationScatter:
    def test_returns_samples(self, client: TestClient):
        body = client.get("/v1/eva/ops/metrics/calibration", headers=DAVE).json()
    ...
    def test_sample_shape(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/calibration?limit=5", headers=DAVE
        ).json()
...
class TestLiveOpsLatency:
    ...
    def test_hour_granularity_respects_hours_param(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/liveops?granularity=hour&hours=6",
            headers=DAVE,
        ).json()
    ...
    def test_invalid_granularity_returns_422(self, client: TestClient):
        resp = client.get(
            "/v1/eva/ops/metrics/liveops?granularity=bogus", headers=DAVE
        )
        assert resp.status_code == 422

    def test_hourly_series_is_contiguous_and_ends_now(self, client: TestClient):
        body = client.get(
            "/v1/eva/ops/metrics/liveops?granularity=hour&hours=3",
            headers=DAVE,
        ).json()
...
class TestIncidents:
    def test_list_returns_rows(self, client: TestClient):
        rows = client.get("/v1/eva/ops/incidents", headers=DAVE).json()
        assert len(rows) >= 1
        assert {"id", "title", "status", "severity", "started_at"} <= set(rows[0])

    def test_status_filter(self, client: TestClient):
        rows = client.get(
            "/v1/eva/ops/incidents?status=resolved", headers=DAVE
        ).json()
        assert all(r["status"] == "resolved" for r in rows)

    def test_invalid_status_returns_422(self, client: TestClient):
        resp = client.get(
            "/v1/eva/ops/incidents?status=bogus", headers=DAVE
        )
        assert resp.status_code == 422

    def test_requires_ops(self, client: TestClient):
        resp = client.get("/v1/eva/ops/incidents", headers=ALICE)
        assert resp.status_code == 403
