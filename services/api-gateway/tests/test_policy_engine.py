    ...
    rules = [
        {
            ...
            "conditions": [
                ...
                {"field": "region", "operator": "in", "value": ["eastus", "westus"]},
            ],
            "pass_action": "allow",
            "fail_action": "block",
        },
...
class TestSensitiveBoundary:
    """Test ORG-GEN-AI-01 sensitive boundary enforcement."""

    def test_sensitive_passes_with_primary_region(self, policy_engine):
        """sensitive with primary region region passes."""
        context = {
            "data_classification": "sensitive",
            "region": "eastus",
        }
        result = policy_engine.evaluate(context)
        # Should pass because sensitive in primary region
        assert any(v.rule_id == "ORG-GEN-AI-01" and v.passed for v in result.verdicts)

    def test_sensitive_passes_with_secondary_region(self, policy_engine):
        """sensitive with secondary region region passes."""
        context = {
            "data_classification": "sensitive",
            "region": "westus",
        }
    ...
    def test_sensitive_blocks_with_useast(self, policy_engine):
        ...
        result = policy_engine.evaluate(context)
        # Should fail because sensitive outside approved regions
        verdict = next((v for v in result.verdicts if v.rule_id == "ORG-GEN-AI-01"), None)
...
class TestRagGrounding:
    """Test ORG-GEN-AI-02 RAG-first grounding enforcement."""
    ...
    def test_grounding_passes_at_ratio_50_percent(self, policy_engine):
        ...
        result = policy_engine.evaluate(context)
        verdict = next((v for v in result.verdicts if v.rule_id == "ORG-GEN-AI-02"), None)
        assert verdict is not None
...
class TestDataResidency:
    ...
    def test_residency_primary_region_passes(self, policy_engine):
        """primary region residency passes."""
        context = {
            "tool_data_residency": "eastus",
        }
        result = policy_engine.evaluate(context)
        verdict = next(
    ...
    def test_residency_secondary_region_passes(self, policy_engine):
        """secondary region residency passes."""
        context = {
            "tool_data_residency": "westus",
        }
        result = policy_engine.evaluate(context)
        verdict = next(
            (v for v in result.verdicts if v.rule_id == "Organization-RESIDENCY-01"), None
    ...
    def test_residency_us_shorthand_passes(self, policy_engine):
        """US shorthand passes."""
        context = {
            "tool_data_residency": "us",
        }
        result = policy_engine.evaluate(context)
        verdict = next(
            (v for v in result.verdicts if v.rule_id == "Organization-RESIDENCY-01"), None
...
class TestAuditLogging:
    ...
    def test_audit_logs_on_evaluation(self, policy_engine, caplog):
        ...
        context = {"data_classification": "sensitive", "region": "eastus"}
...
class TestOperators:
    ...
    def test_in_operator(self, policy_engine):
        ...
        assert _OPS["in"]("eastus", ["eastus", "westus"]) is True
        assert _OPS["in"]("useast", ["eastus", "westus"]) is False
