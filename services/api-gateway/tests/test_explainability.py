...
class TestReasoningSummarizer:
    ...
    def test_query_appears_in_summary(self):
        result = self.summarizer.summarize(
            search_query="the social benefits Act section 5",
            sources_consulted=1,
            sources_used=1,
            sources_excluded=0,
            exclusion_reasons=[],
        ...
        assert "the social benefits Act section 5" in result
...
class TestNegativeEvidenceDetector:
    ...
    def test_no_results_returns_early(self):
        ...
        negatives = self.detector.detect(
            user_query="What does the labor benefits Act say about 2025?",
            search_results=[],
    ...
    def test_missing_act_reference(self):
        negatives = self.detector.detect(
            user_query="What does the labor benefits Act say about parental leave?",
            search_results=[{"content": "Parental leave info", "file": "leave.pdf"}],
            answer_text="Parental leave is available for eligible employees.",
        )
        assert any("labor benefits Act" in n for n in negatives)

    def test_act_reference_found(self):
        negatives = self.detector.detect(
            user_query="What does the labor benefits Act say?",
            search_results=[
                {"content": "Under the labor benefits Act, section 12...", "file": "ei.pdf"}
            ],
            answer_text="The labor benefits Act provides benefits.",
        )
        assert not any("labor benefits Act" in n for n in negatives)
    ...
    def test_multiple_negatives(self):
        negatives = self.detector.detect(
            user_query="What does the national Labour Code say about 2025 amendments?",
            search_results=[{"content": "General labour info from 2023", "file": "labour.pdf"}],
            answer_text="The rules may apply to certain employees.",
        )
        # Should detect: hedging ("may"), missing date (2025), missing Act (national Labour Code)
