"""Tests for POST /v1/eva/admin/tenants/init (Phase A — Onboarding wizard wiring)."""
...
BODY = {
    ...
    "invitees": ["alice@acme.gc.ca", "bob@acme.gc.ca"],
...
class TestTenantInit:
    def test_creates_client_and_interview(self, client: TestClient):
        resp = client.post(
            "/v1/eva/admin/tenants/init", headers=CAROL, json=BODY
        )
    ...
    def test_non_admin_is_forbidden(self, client: TestClient):
        resp = client.post(
            "/v1/eva/admin/tenants/init", headers=BOB, json=BODY
        )
        assert resp.status_code == 403

    def test_invalid_classification_returns_422(self, client: TestClient):
        resp = client.post(
            "/v1/eva/admin/tenants/init",
            headers=CAROL,
    ...
    def test_audit_trail_recorded(self, client: TestClient):
        client.post("/v1/eva/admin/tenants/init", headers=CAROL, json=BODY)
        entries = audit_store.query(action="tenant.init")
        assert len(entries) == 1
        assert entries[0].subject == "Acme Public Sector"
        assert entries[0].decision == "allow"

    def test_decision_informing_maps_to_aicm_level_2(self, client: TestClient):
        resp = client.post(
            "/v1/eva/admin/tenants/init",
            headers=CAROL,
        ...
        assert matching[0].aicm_assessment == "level_2"
