...
CAROL = {"x-demo-user-email": "carol@demo.gc.ca"}
...
class TestArchetypesCatalog:
    def test_list_returns_five_seeded_archetypes(self):
        resp = client.get("/v1/eva/archetypes", headers=CAROL)
    ...
    def test_requires_auth(self):
        """No demo header → 401."""
        resp = client.get("/v1/eva/archetypes")
        assert resp.status_code == 401

    def test_get_by_key(self):
        resp = client.get("/v1/eva/archetypes/policy", headers=CAROL)
        assert resp.status_code == 200
        assert resp.json()["key"] == "policy"
        assert resp.json()["assurance"] == "Decision-informing"

    def test_get_unknown_key_returns_404(self):
        resp = client.get("/v1/eva/archetypes/not-a-real-key", headers=CAROL)
