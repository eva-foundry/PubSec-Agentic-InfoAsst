...
Reference: Organization RBAC model (Reader / Contributor / Admin) + portal_access grants.
"""
...
BOB = _h("bob@example.org")       # reader, self-service only, ws-oas-act
ALICE = _h("alice@example.org")   # contributor, self-service only, ws-oas-act + ws-ei-juris
CAROL = _h("carol@example.org")   # admin, self-service + admin, all workspaces
DAVE = _h("dave@example.org")     # admin, self-service + admin + ops, all workspaces
EVE = _h("eve@example.org")       # contributor, self-service only, ws-oas-act + ws-ei-juris + ws-bdm-km
...
class TestReaderRestrictions:
    ...
    def test_reader_can_list_workspaces(self):
        ...
        r = client.get("/v1/aia/workspaces", headers=BOB)
    ...
    def test_reader_can_view_documents(self):
        ...
        r = client.get("/v1/aia/documents/status", headers=BOB)
        assert r.status_code == 200

    def test_reader_cannot_access_admin_clients(self):
        """Admin client listing requires admin role."""
        r = client.get("/v1/aia/admin/clients", headers=BOB)
    ...
    def test_reader_cannot_access_admin_models(self):
        r = client.get("/v1/aia/admin/models", headers=BOB)
        assert r.status_code == 403

    def test_reader_cannot_access_admin_prompts(self):
        r = client.get("/v1/aia/admin/prompts", headers=BOB)
        assert r.status_code == 403

    def test_reader_cannot_access_ops_health(self):
        """Ops health requires ops portal_access."""
        r = client.get("/v1/aia/ops/health", headers=BOB)
        assert r.status_code == 403

    def test_reader_cannot_access_ops_finops(self):
        r = client.get("/v1/aia/ops/metrics/finops", headers=BOB)
        assert r.status_code == 403

    def test_reader_cannot_access_ops_aiops(self):
        r = client.get("/v1/aia/ops/metrics/aiops", headers=BOB)
        assert r.status_code == 403

    def test_reader_cannot_access_ops_liveops(self):
        r = client.get("/v1/aia/ops/metrics/liveops", headers=BOB)
        assert r.status_code == 403
...
class TestContributorRestrictions:
    ...
    def test_contributor_cannot_access_admin_clients(self):
        r = client.get("/v1/aia/admin/clients", headers=ALICE)
        assert r.status_code == 403

    def test_contributor_cannot_access_admin_models(self):
        r = client.get("/v1/aia/admin/models", headers=ALICE)
        assert r.status_code == 403

    def test_contributor_cannot_access_ops_health(self):
        r = client.get("/v1/aia/ops/health", headers=ALICE)
        assert r.status_code == 403

    def test_contributor_cannot_access_ops_finops(self):
        r = client.get("/v1/aia/ops/metrics/finops", headers=ALICE)
        assert r.status_code == 403
...
class TestAdminAccess:
    ...
    def test_admin_can_list_clients(self):
        r = client.get("/v1/aia/admin/clients", headers=CAROL)
        assert r.status_code == 200
    ...
    def test_admin_can_list_models(self):
        r = client.get("/v1/aia/admin/models", headers=CAROL)
        assert r.status_code == 200

    def test_admin_can_list_prompts(self):
        r = client.get("/v1/aia/admin/prompts", headers=CAROL)
        assert r.status_code == 200

    def test_admin_can_list_workspaces(self):
        r = client.get("/v1/aia/workspaces", headers=CAROL)
        assert r.status_code == 200

    def test_admin_cannot_access_ops_health(self):
        """Carol has admin portal access but NOT ops."""
        r = client.get("/v1/aia/ops/health", headers=CAROL)
        assert r.status_code == 403

    def test_admin_cannot_access_ops_finops(self):
        r = client.get("/v1/aia/ops/metrics/finops", headers=CAROL)
        assert r.status_code == 403
...
class TestOpsAccess:
    ...
    def test_ops_can_access_health(self):
        r = client.get("/v1/aia/ops/health", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_access_finops(self):
        r = client.get("/v1/aia/ops/metrics/finops", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_access_aiops(self):
        r = client.get("/v1/aia/ops/metrics/aiops", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_access_liveops(self):
        r = client.get("/v1/aia/ops/metrics/liveops", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_view_traces(self):
        r = client.get("/v1/aia/ops/traces/conv-123", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_view_corpus_health(self):
        r = client.get("/v1/aia/ops/corpus-health", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_view_deployments(self):
        r = client.get("/v1/aia/ops/deployments", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_view_evaluation_arena(self):
        r = client.get("/v1/aia/ops/evaluation-arena", headers=DAVE)
        assert r.status_code == 200

    def test_ops_can_also_access_admin(self):
        """Dave has both admin and ops portal access."""
        r = client.get("/v1/aia/admin/clients", headers=DAVE)
        assert r.status_code == 200
...
class TestNoAuth:
    ...
    def test_no_header_returns_401(self):
        r = client.get("/v1/aia/workspaces")
        assert r.status_code == 401

    def test_invalid_email_returns_401(self):
        r = client.get("/v1/aia/workspaces", headers=_h("nobody@example.org"))
        assert r.status_code == 401

    def test_empty_email_returns_401(self):
        r = client.get("/v1/aia/workspaces", headers=_h(""))
        assert r.status_code == 401

    def test_no_auth_on_chat(self):
        r = client.post("/v1/aia/chat", json={"message": "hello", "workspace_id": "ws-oas-act"})
        assert r.status_code == 401

    def test_no_auth_on_admin(self):
        r = client.get("/v1/aia/admin/clients")
        assert r.status_code == 401

    def test_no_auth_on_ops(self):
        r = client.get("/v1/aia/ops/health")
        assert r.status_code == 401

    def test_no_auth_on_documents(self):
        r = client.get("/v1/aia/documents/status")
        assert r.status_code == 401
